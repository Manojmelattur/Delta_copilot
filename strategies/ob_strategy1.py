...
class OrderBlockStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        trend_ema_period: int = 50,  # NEW: EMA period for trend filter
        min_atr_threshold: float = 0.0,  # NEW: minimum ATR to allow entry (0 = disabled)
        ...
        self.trend_ema_period = trend_ema_period  # NEW
        self.min_atr_threshold = min_atr_threshold  # NEW
        ...
        self._diag_trend_filtered = 0  # NEW: count entries skipped by EMA filter
        self._diag_atr_filtered = 0  # NEW: count entries skipped by ATR threshold
    ...
    def get_params(self) -> dict:
        return {
            ...
            "trend_ema_period": self.trend_ema_period,  # NEW
            "min_atr_threshold": self.min_atr_threshold,  # NEW
        }
    ...
    def get_signal(self, candle: dict, indicators: dict) -> str | None:
        ...
        trend_ema = indicators.get("trend_ema")  # NEW
        ...
        # --- NEW: Minimum ATR threshold filter ---
        if self.min_atr_threshold > 0 and atr < self.min_atr_threshold:
            self._diag_atr_filtered += 1
            return None

        # --- NEW: Trend EMA - NaN is treated as filter disabled for that candle ---
        ...
        if self._pending_ob_bias is not None:
            ...
            if self._pending_ob_bias == "long" and ob_l <= close <= ob_h:
                # NEW: EMA trend filter - only take longs above trend EMA
                ...
                # NEW: EMA trend filter - only take shorts below trend EMA
                if trend_ema_val is not None and close > trend_ema_val:
                    logger.debug(
        ...
        if bool(ob_bear) and ob_bear_high is not None and not pd.isna(ob_bear_high):
    ...
    def reset_state(self) -> None:
        ...
        self._diag_retracement_checks = 0
        self._diag_entries_fired = 0
        self._diag_trend_filtered = 0  # NEW
        self._diag_atr_filtered = 0  # NEW
    ...
    def get_exit_signal(
        ...
        if side == "buy":
            ...
            if high >= tp_price and close <= sl_price:
                logger.warning(
                    f"[OB] Ambiguous exit on BUY: high={high} >= tp={tp_price:.4f} "
                    f"AND close={close} <= sl={sl_price:.4f}. TP wins. entry={entry_price}"
                )

            if high >= tp_price:
                return "sell", "EXIT_TP"
            if close <= sl_price:  # close-based SL
                return "sell", "EXIT_SL"
            if trail_activated:
            ...
            if low <= tp_price and close >= sl_price:
                logger.warning(
                    f"[OB] Ambiguous exit on SELL: low={low} <= tp={tp_price:.4f} "
                    f"AND close={close} >= sl={sl_price:.4f}. TP wins. entry={entry_price}"
                )
